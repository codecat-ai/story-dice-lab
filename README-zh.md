...
行为测试位于 `tests/storyDice.test.ts`，覆盖确定性结果、锁定行为、单骰重掷、提示格式化、五拍场景大纲导出、工作坊讲义导出、主持人议程导出、议程计时卡片导出和带裁切线的可打印计时卡片版式、课堂修订卡导出和带裁切线的可打印修订卡版式及其可编辑标题控件、时间缩放和 UI 控件、Markdown 提示导出、打印布局格式化、浏览器打印触发，以及自定义词库导入/导出。
