...
- Save up to 10 recent local session snapshots in browser history, review newest-first timestamps and titles, copy a concise history list, or clear the history.
...
Local session history stores the 10 most recent saved snapshots in this browser only. Saved items show newest first with ISO timestamps and concise titles; the copied history list is plain text for quick records before exporting or clearing.
...
- Add optional import/export for saved local session history so facilitators can move browser-only records between devices.
