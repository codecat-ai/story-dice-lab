...
- Copy and paste versioned local template packs so departments or writing centers can share classroom templates and word-bank presets without accounts or network access; imported classroom templates are saved in this browser when storage is available.
...
- Save up to 10 recent local session snapshots in browser history with optional facilitator tags and archive labels, filter records by tag or label, search titles and snapshot Markdown text in browser-only UI state, view a saved snapshot detail panel for copy/print reuse without changing the current roll, copy a concise visible history list, copy portable visible history JSON, print a compact batch sheet for the currently visible filtered set, import validated history JSON from another browser, or clear the history.
...
Behavior tests live in `tests/storyDice.test.ts`, `tests/sessionSnapshot.test.ts`, `tests/sessionHistory.test.ts`, `tests/classroomSessionTemplates.test.ts`, `tests/templatePacks.test.ts`, `tests/wordBankPresets.test.ts`, and `tests/keyboardShortcuts.test.ts`. They cover deterministic rolls, lock behavior, single-die rerolls, prompt formatting, five-beat scene outline export, workshop handout export, facilitator agenda export, agenda timer-card export and printable timer-card cut-line layout, large-type facilitator timer phase state and accessible display controls, classroom revision card export and printable revision-card cut-line layout with editable title controls, small-group peer role-card export and printable peer role-card cut-line layout with editable title controls, compact post-critique action-plan export with editable title controls, deterministic Markdown session snapshot formatting, compact printable session snapshot HTML with escaped and bounded user content, injected session snapshot print targeting, local session history ordering, max-10 trimming, facilitator tag normalization and filtering, archive-label normalization and filtering, title/content search filtering, visible-list and visible-JSON filtering, visible-history print batch formatting without hidden-record leaks, legacy untagged/unlabeled history loading, deterministic tagged and labeled history JSON export, validated history JSON import, import replacement behavior, corrupted/unavailable storage fallbacks, history tag/label/search/import/export/detail/print control markup, selected saved snapshot detail rendering, friendly empty detail states, time scaling and UI controls, Markdown prompt export, print-layout formatting, browser print triggering, classroom session template listing/application/control rendering including caller-provided imported templates, deterministic template-pack export, validated template-pack import, optional local persistence and merge behavior for imported classroom templates, template-pack control markup, custom word-bank import/export, local word-bank preset storage with optional notes, and pure keyboard shortcut mapping plus typing-field suppression.

## Roadmap

Maturity: `growth`. Planned cadence: 1-2 focused sessions per week while classroom/workshop workflows are still being hardened; review for `maintenance` after backup, reflection, and template-pack persistence workflows are complete.

- Add a browser-only backup reminder for facilitators before clearing local records.
- Add printable facilitator reflection prompts for turning saved histories into next-session goals.
- Add a short demo dataset/workshop walkthrough that connects imported packs, saved histories, and facilitator exports.
